...
export const useSOSFUProcesses = () => {
  ...
  const getCategory = (process: Process): ProcessCategory => {
    ...
    // Hardened PC Check - Priority 1 (Workflow)
    if (
      workflow.includes('PC_') || 
      workflow.includes('ACCOUNTABILITY') ||
      process.type === ProcessType.ACCOUNTABILITY
    ) {
      return 'PRESTACAO';
    }

    // Secondary PC Check - Status Keywords
    if (
      status.includes('PRESTAND') || 
      status.includes('AGUARDANDO PC') ||
      status.includes('PC ') ||
      (status === 'EM_ANALISE' && workflow === 'PC_REVIEW_SOSFU') ||
      status.includes('PRESTACAO') ||
      status.includes('PRESTAÇÃO')
    ) {
      return 'PRESTACAO';
    }
