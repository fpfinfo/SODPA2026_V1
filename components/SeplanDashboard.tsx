...
interface SeplanDashboardProps {
  processes?: Process[];
...
export const SeplanDashboard: React.FC<SeplanDashboardProps> = ({ processes = [], onSignComplete }) => {
  ...
  const dynamicTasks: SigningTask[] = useMemo(() => {
    return processes
      .filter(p => p.status === ConcessionStatus.SIGNATURE)
      .map(p => ({
        id: `PROC-${p.id}`,
  ...
  return (
    <div className="h-full bg-[#f8fafc] overflow-hidden flex flex-col relative">
      <div className="bg-white border-b border-slate-200 px-10 py-8 flex justify-between items-end shadow-sm z-20">
        <div><h1 className="text-3xl font-black text-slate-900 tracking-tighter flex items-center gap-3">Módulo Executivo <span className="px-3 py-1 bg-slate-100 rounded-lg text-xs font-medium text-slate-500 tracking-normal border border-slate-200">SEPLAN</span></h1><p className="text-slate-500 font-medium mt-2 flex items-center gap-2"><Briefcase size={16} className="text-slate-400" /> Painel do Ordenador de Despesas</p></div>
