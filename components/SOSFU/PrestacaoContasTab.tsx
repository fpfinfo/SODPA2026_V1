...
import { SefinAnaliseModal } from './SefinAnaliseModal';
...
export const PrestacaoContasTab: React.FC<PrestacaoContasTabProps> = ({
  ...
  const periodInfo = useMemo(() => {
    ...
    // Período de Aplicação: Da emissão da Portaria até a data final do evento
    // FIX: Se dataFim for anterior à Portaria (inconsistência ou erro), usar data da Portaria como base
    const effectiveBaseDate = (portariaDate && dataFimDate && dataFimDate < portariaDate) 
       ? portariaDate 
       : (dataFimDate || portariaDate);

    let diasAplicacao = 0;
    if (portariaDate && effectiveBaseDate) {
      diasAplicacao = Math.ceil((effectiveBaseDate.getTime() - portariaDate.getTime()) / (1000 * 60 * 60 * 24));
    }
    
    // Prazo Unificado de Aplicação e Prestação de Contas: Data base + 15 dias (Art. 4°)
    const prazoUnificadoDate = effectiveBaseDate ? new Date(effectiveBaseDate.getTime() + 15 * 24 * 60 * 60 * 1000) : null;
    ...
    return {
      ...
      dataFimDate,
      diasAplicacao,
      prazoPCDate: prazoUnificadoDate, // Mantém nome para retrocompatibilidade
      periodoAplicacaoOk: portariaDate && effectiveBaseDate ? (diasAplicacao >= 0 && diasAplicacao <= 90) : null,
      periodoAplicacaoLabel: portariaDate && effectiveBaseDate 
        ? `${formatDate(portariaDate)} → ${formatDate(effectiveBaseDate)} (${diasAplicacao} dias)`
        : 'Datas não disponíveis',
      prazoPCLabel: prazoUnificadoDate 
        ? `até ${formatDate(prazoUnificadoDate)} (data final + 15 dias)`
        : 'Data não disponível'
    };
  ...
  useEffect(() => {
    const checkAtesto = async () => {
      ...
      // Use case-insensitive logic via explicit list
      // CRITICAL: Use limit(1) because maybeSingle fails if multiple exist (Concession + PC)
      ...
      setHasAtestoGestor(!!data && data.length > 0);
    };
    checkAtesto();
  }, [solicitacaoId]);
  ...
  const autoValidation = useMemo(() => {
    // 2. Prazo PC: PC submetida antes de dataFim + 15 dias (prazo unificado Art. 4°)
    ...
    // Permite sub-elementos (ex: 3.3.90.30.01) checando prefixo
    const ELEMENTOS_PERMITIDOS = ['3.3.90.30', '3.3.90.33', '3.3.90.36', '3.3.90.39'];
    const elementosOk = comprovantes.length > 0 
      ? comprovantes.every(c => ELEMENTOS_PERMITIDOS.some(prefix => c.elemento_despesa.startsWith(prefix)))
      : null;
  ...
  const checklistCount = useMemo(() => {
    ...
    const completed = items.filter(item => 
      item.auto === true || item.auto === null || checklist[item.key as keyof typeof checklist]
    ).length;
  ...
  const failureDetails = useMemo(() => {
    ...
    if (autoValidation.prazoPCOk === false) {
      failures.push({
        ...
        description: 'A PC foi submetida após o prazo de aplicação e prestação de contas (data final + 15 dias).',
        severity: 'high',
        actions: [
  ...
  const executeApproval = async () => {
    ...
    setIsAnaliseModalOpen(false); // Close modal
    ...
    try {
      ...
      if (saldoDevolver > 0 && pc?.gdr_saldo_paga) {
        await supabase.from('gestao_devolucoes').insert({
          ...
          status_gdr: 'PENDENTE',
          data_referencia: new Date().toISOString() // Required field
        });
      }
    ...
    <div className="space-y-6">
            ...
            {[
              ...
              { 
                ...
                sublabel: autoValidation.prazoPCOk === false 
                  ...
                  : 'Data final da solicitação + 15 dias (Art. 4°)',
                required: true,
      ...
      {canReview && (
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          {pc?.status === 'AGUARDANDO_ATESTO_GESTOR' ? (
            <div className="w-full flex justify-end">
              <div className="px-6 py-3 bg-slate-100 border border-slate-200 text-slate-500 rounded-xl text-sm font-bold flex items-center gap-2 cursor-not-allowed opacity-70">
                <Clock size={18} />
                Aguardando Atesto do Gestor
              </div>
            </div>
          ) : (
            <>
      ...
      <SefinAnaliseModal
        ...
        processData={{
          nup: processData.nup,
          supridoNome: processData.supridoNome
        }}
        financialData={{
          valorConcedido: processData.valorConcedido,
          totalGasto: totalLiquido,
          totalGlosado: totalGlosado,
          saldoDevolver: Math.max(0, processData.valorConcedido - totalLiquido)
        }}
        hasGDR={pc?.gdr_saldo_paga}
        hasAtesto={hasAtestoGestor}
      />
