...
export const GestorDashboard: React.FC = () => {
  ...
  // Check if process has Certidão de Atesto (enables Tramitar button)
  // STRICT CHECK: Only accepts exact types generated by the system
  ...
  console.log('[DEBUG] Atesto Check:', { 
      isAutoAtesto, 
      hasAtesto, 
      currentUserId, 
  ...
  // Show "Gerar Atesto" button when:
  // 1. Process is pending atesto
  // 2. AND (still loading docs OR doesn't already have an atesto document)
  const needsAtesto = isInitialAtesto && (isLoadingDossierDocs || !hasAtesto);
  
  // Can tramitar if: has atesto document (MANDATORY for everyone, even Auto-Atesto)
  const canTramitarToSOSFU = !isLoadingDossierDocs && hasAtesto;
