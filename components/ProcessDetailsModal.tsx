  ...
  const getFooterActions = () => {
    ...
    return (
      <div className="flex gap-4">
        ...
        <button disabled={!isApproved} className={`px-8 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-xl flex items-center gap-2 ${isApproved ? 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-emerald-500/30' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}><CheckCircle size={18} /> Aprovar e Encaminhar (SEPLAN)</button>
      </div>
    );
  };
