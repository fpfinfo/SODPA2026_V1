...
import { DocumentPreview } from './SEFIN/DocumentPreview';
...
interface SigningTask {
  id: string;
  protocol: string;
  type: string; // Allow any document type
...
export const SefinDashboard: React.FC<SefinDashboardProps> = ({ processes = [], onSignComplete }) => {
  ...
  const filteredTasks = allTasks.filter(t => {
    ...
    if (activeTab === 'SIGNED' && (t.status !== 'SIGNED' && t.status !== 'SENT')) return false; // Show both Signed and Sent (History)
    ...
    <div className="h-full bg-[#f8fafc] overflow-hidden flex flex-col relative">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <DocumentPreview
          task={previewTask ? {
            id: previewTask.id,
            protocol: previewTask.protocol,
            type: previewTask.type,
            title: previewTask.title,
            description: previewTask.description,
            value: previewTask.value,
            date: previewTask.date,
            author: previewTask.author,
            content_preview: previewTask.content_preview,
            documento_id: previewTask.documento_id,
            solicitacao_id: previewTask.solicitacao_id,
          } : null}
          onClose={() => setPreviewTask(null)}
          onSign={() => {
            setSelectedIds(new Set([previewTask?.id || '']));
